...
function checkForDuplicates(directory, name, isFile) {
    const sanitizedDirectory = path.normalize(directory).replace(/^(\.\.[\/\\])+/, '').replace(/\\/g, '/');
    var filenames = fs.readdirSync(sanitizedDirectory);
    if (filenames.indexOf(name) == -1) {
        return false;
    } else {
        for (var i = 0; i < filenames.length; i++) {
            if (filenames[i] === name) {
                if (!isFile && fs.lstatSync(sanitizedDirectory + "/" + filenames[i]).isDirectory()) {
                    return true;
                } else if (isFile && !fs.lstatSync(sanitizedDirectory + "/" + filenames[i]).isDirectory()) {
                    return true;
                } else {
                    return false;
                }
...
function checkForFileUpdate(fromPath, toPath, item, contentRootPath, req) {
    ...
    var sanitizedTargetPath = path.normalize(contentRootPath + req.body.targetPath).replace(/^(\.\.[\/\\])+/, '').replace(/\\/g, '/');
    if (fromPath == toPath) {
        if (checkForDuplicates(sanitizedTargetPath, name, item.isFile)) {
            updateCopyName(sanitizedTargetPath, name, count, item.isFile);
        }
    } else {
